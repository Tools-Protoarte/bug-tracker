...
*/
	...
	//login configuration
	if (get_option('zing_bt_mantisbt_dbname')) $wpdb->select(get_option('zing_bt_mantisbt_dbname'));
	if (get_option("zing_bt_login")=="MyBB") {
		$user_login=get_option('zing_bt_admin_login');//$current_user->data->user_login;
		$user_pass=zing_bt_admin_password();//get_option("zing_bt_admin_password");
		$salt=create_sessionid(8);
		$loginkey=create_sessionid(50);
		$password=md5(md5($salt).md5($user_pass));
...
function zing_bt_output($process) {
	...
	switch ($process)
	{
		case "content":
			...
			{
				$zing_bt_to_include=$_GET['zforum'];
				$zing_bt_mode="client";
			}
			elseif (isset($_GET['zforumadmin']))
			{
				$zing_bt_to_include="admin/".$_GET['zforumadmin'];
				$zing_bt_mode="admin";
			}
...
function zing_bt_header()
{
	...
	//echo "function zing_url_enhance(s) { s=s.replace('.php?','&'); return zing_bt_index+'zforum='+s; }";
...
function zing_bt_ob($buffer) {
	...
	//$buffer=str_replace(ZING_MANTIS_URL."/admin/index.php",$home."index.php?page_id=".$pid."&zforumadmin=index",$buffer);
	...
	//$buffer=str_replace('forumdisplay.php?',"index.php?zforum=forumdisplay&",$buffer);
	//$buffer=preg_replace('/([A-Za-z0-9_]*).php.(.*)/','index.php?zforum=$1&$2',$buffer);
	...
	if ($zing_bt_mode=="client") {
		...
		$r[]='"'.$home.'index.php?page_id='.$pid.'&zforum=$1&';
		
		$f[]='/href=\"'.preg_quote($mantisbtself,'/').'\/(.*?).php'.'\"/';
		$r[]='href="'.$home.'index.php?page_id='.$pid.'&zforum=$1"';
		
		$f[]='/'.preg_quote(ZING_MANTIS_URL,'/').'\/(.*?).php\?'.'/';
		$r[]='"'.$home.'index.php?page_id='.$pid.'&zforum=$1&';

		$f[]='/action=\"(.*?).php/';
		$r[]='action="'.$home.'index.php?page_id='.$pid.'&zforum=$1"';
		...
		//$buffer=str_replace($mantisbtself.'/',$home.'?zforum=',$buffer);
		//popup
		...
		foreach ($pages as $page) {
			$buffer=str_replace(ZING_MANTIS_URL."/".$page.".php?",$home."index.php?page_id=".$pid."&zforum=".$page."&",$buffer);
			$buffer=str_replace(ZING_MANTIS_URL."/".$page.".php",$home."index.php?page_id=".$pid."&zforum=".$page,$buffer);
		}
		unset($pages[0]);
		foreach ($pages as $page) {
		...
		*/
		...
		//help
		//$buffer=str_replace($home.'misc.php?action=help',$home."index.php?page_id=".$pid."&zforum=misc&action=help",$buffer);
	} else {
		//admin pages
		/*
		...
		$pages=explode(",",$pageslist);
		foreach ($pages as $page) {
			$buffer=str_replace($mantisbt.$page.".php?",$admin."admin.php?page=bug-tracker-admin&zforumadmin=".$page."&",$buffer);
			$buffer=str_replace($mantisbt.$page.".php",$admin."admin.php?page=bug-tracker-admin&zforumadmin=".$page,$buffer);
			$buffer=str_replace($self.'wp-content/plugins/bug-tracker/'.ZING_MANTIS.'/admin/'.$page.".php?",$admin."admin.php?page=bug-tracker-admin&zforumadmin=".$page.'&',$buffer);
			$buffer=str_replace('../'.$page.'.php?',$admin."admin.php?page=bug-tracker-admin&zforumadmin=".$page."&",$buffer);
		}
		...
		//special cases
		$buffer=str_replace('"index.php"',$admin.'admin.php?page=bug-tracker-admin&zforumadmin=index',$buffer);
		...
		//logout
		$buffer=str_replace('"index.php?action=logout"','"'.$admin.'admin.php?page=bug-tracker-admin&zforumadmin=index&action=logout"',$buffer);
		...
		//redirect form
		$buffer=str_replace('"index.php"','"'.$admin.'admin.php?page=bug-tracker-cp"&zforum=index',$buffer);
...
function zing_bt_login() {
	global $current_user;
	if (is_user_logged_in() && get_option("zing_bt_login")=="WP") {
...
function zing_bt_admin_password() {
	...
	if (get_option("zing_bt_login")=="MyBB") {
		$user_pass=get_option("zing_bt_admin_password");
	} else {
